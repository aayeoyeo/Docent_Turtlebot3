...
## 2. 기능 요구사항 (MoSCoW)
교수님 과제 명세서의 검증 기준과 스타터 코드의 주요 한계점 해결 방안을 명확히 반영하여 요구사항을 정의합니다.
...
## 5. 역할 분담표
본 4조는 5인 조 구성으로 가이드라인에 의거하여 조장이 웹 프런트 레이아웃 개발 및 전체 통합·QA를 겸임합니다. 실질적인 코어 백엔드 소스 코드 개발은 여태인 조원이 전담하고, 웹 및 클라이언트 사이드 AI/통신 제어 핵심 코드는 최준혁 조원이 전담하여 파트별 개발 효율성을 최대로 끌어올립니다. 기여도가 낮은 인원은 명목상의 행정 및 단순 보조 업무로 배정합니다.

| 이름 | 역할 | 담당 범위 (실무 상세) | 주요 산출물 |
|---|---|---|---|
| **김대권** | 🌐 웹 프런트 / 🔗 통합 QA 총괄 (조장) | 관제탑 대시보드 레이아웃 구조 설계 및 UI 컴포넌트 구현, 시스템 E2E 전체 통합 QA 검토 및 토픽 인터페이스 작동 유효성 검증 마스터, 프로젝트 일정 총괄 | `admin.html` 외관 |
| **여태인** | 🤖 ROS 노드 / 🧠 백엔드 로직 개발 | ROS 2 메인 제어 노드 백엔드 소스 코드 실질 코딩, 다단계 상태 머신(MOVE➔EXPLAIN➔CHECK) 아키텍처 구축, 미인식 타임아웃 및 코스 주입 리셋 예외 처리 로직 구현 | `docent_node.py`, `PLAN.md` |
| **최준혁** | 💻 웹 AI 및 클라이언트 사이드 개발 | 모바일 브라우저 최초 제어 활성화를 포함한 스마트폰 통합 페이지 코어 로딩 구현, QR 스캔/`tracking.js` 안면 인식/TTS 오디오 웹 흐름 제어 및 웹소켓(`roslibjs`) 바인딩 데이터 송수신 실질 코딩 | `robot.html` 코어 로직 |
| **진승민** | 🔩 하드웨어 백업 / 회의록 지원 | 하드웨어 장비 구동 전후 단순 케이블 연결 확인 및 주변 정리 보조, 개발 일지 누적 작성을 위한 팀 회의 내용 원시 텍스트 타이핑 기록 | 회의 기록 텍스트 |
| **박현상** | 🎨 콘텐츠 텍스트 검수 / 대본 정리 | 주입용 전시물 JSON 대본 텍스트의 오타 교정 및 단순 자구 수정 보조, 팀 내 마크다운 산출물 서식 정리를 위한 단순 텍스트 입력 지원 | 스크립트 오타 검수 표 |
