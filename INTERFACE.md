...
| `/audience` | `std_msgs/msg/Bool` | 스마트폰 웹앱 (`robot.html`) | 중앙 도슨트 노드 (`docent_node`) | 10Hz (인식 지속 시) | 연속 (Continuous) | **BEST_EFFORT** / Volatile | `tracking.js` 엔진의 안면 유무 판정 결과 (`true`/`false`). 5초 유지 후 자동 리셋 |
...
| `/tour_state` | `std_msgs/msg/String` | 중앙 도슨트 노드 (`docent_node`) | 관제탑 웹앱 (`admin.html`) | 10Hz 및 상태 변화 시 | 연속/엣지 복합 | **RELIABLE** / Volatile | 현재 로봇의 제어 Phase, 가동 인덱스, 총 코스 수, 타겟 작품 정보가 담긴 JSON 문자열 브로드캐스트 |
